class args:
    ...
    weights = './checkpoints/yolov4-416-s'
    size  = 416 
    model = 'yolov4'
    # video = ''  #./data/video/license_plate.mp4
    # output = './detections/gui1.avi'
    #info = True
    #log = './detections/log.csv'
    # tiny = False
    # framework = False
    plate = True
...
import cv2
import datetime, re, pytesseract
import math, dropbox
import numpy as np
import matplotlib.pyplot as plt
from tensorflow._api.v2.compat.v1 import ConfigProto          # changes
from tensorflow._api.v2.compat.v1 import InteractiveSession   # changes
# deep sort imports
...
from speed_calculation import recognize_plate, estimateSpeed
...
flags.DEFINE_string('weights', './checkpoints/yolov4-1k-exp-608',
                    'path to weights file')
flags.DEFINE_integer('size', 608, 'resize images to')
flags.DEFINE_boolean('tiny', False, 'yolo or yolo-tiny')
flags.DEFINE_string('model', 'yolov4', 'yolov3 or yolov4')
flags.DEFINE_string('video','./video.mp4' , 'path to input video or set to 0 for webcam') #'./data/video/2.7K_60fps_60kph.mp4' , 
flags.DEFINE_string('output', './detections/gui.avi', 'path to output video')
...
flags.DEFINE_boolean('mask', False, 'do you want to have mask')
flags.DEFINE_string('mask_path', './data/mask1.png', 'path to mask image')
flags.DEFINE_boolean('crop', False, 'crop detections from images')
flags.DEFINE_boolean('plate', False, 'perform license plate recognition')
#flags.DEFINE_string('log', './detections/log.csv', 'path to log file')
...
def main(_argv):
    ...
    while True:
        ...
        allowed_classes = ['Car',  'Vehicle registration plate']
        ...
        # draw bbox on screen
            ...
            # image = utils.draw_bbox(frame, pred_bbox) #calling draw_bbox function
...
# =============================================================================
        ...
        # #   getting croped images through bbox coordinates and saving them in an empty list
        #     xmin, ymin, xmax, ymax = bbox
        #     box = frame[int(ymin)-20:int(ymax+20), int(xmin)-20:int(xmax)+20]
        # #   large_img = cv2.resize(box, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
        # #   blur = cv2.GaussianBlur(box, (5, 5), 0) #smoothen the image
        #     speeds.append(box)
        #     cv2.imshow('gaari', box)
        #     i=1
        #     for car in speeds:
        #         cv2.imshow(speeds[i])
        #         i+=1
                ...
                #         my_writer.writerow(j)
                        #my_writer.writerow(zip(cars_list,frame))
        ...
        # if enable info flag then print details about each track
            if FLAGS.info:
                ...
                print("Tracker ID: {}, Class: {},  Vehicle registration number: {}, Speed:{}".format(str(track.track_id), class_name, speed_estimation_module, recognize_plate))
